...
export function ChipsVariant({
  ...
  const allUnreadCount = getItemsForView('all').filter(item => item.unread).length;
  ...
  return (
    <section className={styles.group}>
      <div className={styles.chipRow} aria-label={t('inbox.filters.aria')}>
        ...
        >
          {t('inbox.allUpdates')}
          {allUnreadCount ? <span className={styles.chipCount}>{allUnreadCount}</span> : null}
        </button>
        {INBOX_CATEGORIES.map(category => {
          const count = getItemsForView(category.view).filter(item => item.unread).length;
