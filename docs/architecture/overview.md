...
- `packages/` — `eslint-config`, `typescript-config`, `ui`, and **`db`** exist. `@stride/db` (added 2026-06-02) holds the Drizzle schema + drizzle-zod types — 17 tables covering identity/tenancy (`users`, `workspaces`, `memberships`, `teams`, `teamMembers`), sources (`sourceConnections`, `teamSourceMappings`), work (`specs`, `specLinks`, `specActivity`, `actions`, `sessions`, `captures`), schedule (`scheduledEvents`), inbox (`notifications`), and infra (`processedMutations`, `conceptLinks`). Backlog `readiness`/views and `attention` flags are **derived**, not stored. `pnpm db:generate` produces the migration SQL. **To do:** add `api-client`, `queue` when they're needed (per the roadmap — not all at once).
