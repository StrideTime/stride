...
- `packages/` — `eslint-config`, `typescript-config`, `ui`, and **`db`** exist. `@stride/db` (added 2026-06-02) holds the Drizzle schema + drizzle-zod types for identity/tenancy (`users`, `workspaces`, `memberships`, `teams`, `teamMembers`), sources (`sourceConnections`, `sourceUnits`, `teamSourceMappings`), work (`specs`, `specLinks`, `specActivity`, `actions`, `sessions`, `sessionNotes`), schedule (`scheduledEventTypes`, `scheduledEvents`), and inbox (`notifications`). Workspace-owned tables opt into Postgres RLS in the Drizzle schema; API code must set the workspace session context inside the transaction wrapper after validating membership. Backlog `readiness`/views and `attention` flags are **derived**, not stored. `pnpm db:generate` produces the migration SQL. **To do:** add `api-client`, `queue` when they're needed (per the roadmap — not all at once).
