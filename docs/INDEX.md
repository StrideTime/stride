...
- [`architecture/jira-source-mapping-findings.md`](architecture/jira-source-mapping-findings.md) — findings from the Jira API discovery spike: OAuth connection flow, Jira board/config limitations, current schema gaps, and Settings source-mapping changes needed before production Jira sync.
