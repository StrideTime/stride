...
| **Schedule block / entry** | An item placed on the Schedule calendar. `kind` ∈ {`action`, `meeting`, `focus`, `break`}, each with its own icon + color (the `BLOCK_KINDS` registry). | `focus` = protected deep-work block. |
