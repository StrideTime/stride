...
| **Source** / **source system** | Jira, Linear, or GitHub. Stride keeps source-owned status, priority, and difficulty values separate from Team Source mapping badge metadata. | GitHub eventually sends issues + PRs in (configured per-connection) — future. |
...
| **Role** | Scoped permission on either a Workspace Membership or Team Membership: `member` or `admin`. | A person can be a workspace admin, a team admin, both, or neither through separate rows. Admins see the same task views as members, plus approve/edit and transfer-approval where their scope allows. |
...
| **Schedule event type** | A category for planned or actual calendar time. Used for block styling, filtering, capacity reporting, and insights. | Deleting a type means archiving/hiding it from future use; historical blocks/sessions keep their type for accurate reporting. |
| **Working mode** | An app-wide, account-level setting for how the user works: **session-first** or **schedule-first**. Reshapes Today, the Schedule, and time accounting. | Session-first: recorded Sessions are the source of truth for time spent. Schedule-first: planned schedule time counts as worked time. A personal setting, never a team default. See [`overview.md`](overview.md) → User modes. |
