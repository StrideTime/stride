...
4. ~~**Do "Later" / "Snoozed" / "Archive" / "Reassign" collapse?**~~ — **[resolved 2026-06-02; status model superseded 2026-06-03]** The premise was stale prototype-era text; none of those shelf states exist in the built backlog. The backlog organizes by **derived views** (Breakdown / In Flight / Blocked / Next Up / Completed) computed from primitives and source-mapped status. Nothing to collapse. Recorded in [`data-model.md`](data-model.md) and the 2026-06-03 ADR.
...
6. ~~**Label/tag grouping.**~~ — **[resolved 2026-06-04]** Keep `sourceCycle` scoped to sprint/cycle only. Beef up Spec `labels[]` into source-provenance labels (`label | epic | initiative | milestone | project | component`) so epic/initiative/milestone-style grouping can be represented without pretending it is a cycle.
...
8. ~~**Reopen behavior.**~~ — **[superseded 2026-06-03]** Specs no longer carry an internal `open | closed` status. Reopen/close behavior is a source-mapped status change through the source vocabulary; Stride stores `sourceStatus` and mapped status metadata for filters/derived views. Existing Actions, Sessions, and past ScheduledEvents remain immutable history. See [`data-model.md`](data-model.md).
...
- **2026-05-30 — Team General settings narrowed.** Team admin General settings now cover the Stride team name, new synced spec destination, triage owner, missing-estimate behavior, unassigned-work behavior, ready-to-schedule rule, and stale-breakdown nudge. Source-owned identifiers and source-to-team assignment live in Source mapping, not Team General. They do not seed or override member working hours, working mode, notification timing, presence, or focus status; those remain personal settings per `principles.md`.
